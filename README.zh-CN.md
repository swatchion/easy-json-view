...
- **桌面**：单个 JSON 文件 `~/.config/easy-json-view/store.json`，内含 `easy_json_view_history` 与 `easy_json_view_config` 两个键。
- **Web**：浏览器 `localStorage`，同样两个键。
- `easy_json_view_history` — 历史列表（按内容去重，最多 100 条；默认名称为内容 SHA1 的前 7 位短 hash，记录创建时间，书签永不淘汰）。
- `easy_json_view_config` — 格式化选项（缩进 + 排序键）与 UI 设置（主题 / 字号 / 自动格式化）。
