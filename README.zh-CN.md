...
- **数据存储**：桌面 `~/.config/easy-json-view/store.json` 单文件 / Web 浏览器 `localStorage`（由 `src/platform/` 垫片按平台路由）
...
Web 静态产物位于 `target/dx/easy-json-view/release/web/public/`，可用任意静态服务器托管。
...
- `services/mod_enhanced.rs`（平台无关）：
  ...
  - `HistoryService` — 历史 CRUD（key=`easy_json_view_history`，去重、上限 100）
  - `ConfigService` — 配置读写（key=`easy_json_view_config`）
...
- **桌面**：操作系统配置目录下的单个 JSON 文件 `store.json`（路径随 `dirs::config_dir()` 而异），内含 `easy_json_view_history` 与 `easy_json_view_config` 两个键：
...
- `easy_json_view_history` — 历史列表（按内容去重；默认名称为内容 SHA1 的前 7 位短 hash，记录创建时间，书签永不淘汰）。正常保存上限 100 条；**导入 `.zip` 可超过此上限**——导入批量全部保留，并以该更大规模作为后续 FIFO 轮换的高水位。
- `easy_json_view_config` — 格式化选项（缩进 + 排序键）与 UI 设置（主题 / 字号 / 自动格式化 / 语言 / 密度 / 行号）。

**跨设备同步**：桌面历史是单个纯文本 `store.json`，可用 git 版本化/同步——在该配置目录 `git init`（或将 `store.json` 纳入已同步的仓库）。或用桌面端历史侧栏的 **导出 / 导入（.zip）** 按钮在设备间迁移历史（导入会按内容去重并合并进当前会话）。
