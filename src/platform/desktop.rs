...
//!
//! 形如 `~/.config/eazy-json-view/store.json`，内部是一个 `{ key: value }` 映射，
//! `eazy_json_view_history` / `eazy_json_view_config` 各占一个键——天然满足「单文件」要求。
//!
...
/// `store.json` 的完整路径：<config_dir>/eazy-json-view/store.json
fn store_path() -> Result<PathBuf> {
    let mut dir = dirs::config_dir().context("无法定位用户配置目录")?;
    dir.push("eazy-json-view");
